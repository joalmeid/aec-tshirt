...
export const createScene = async function () {
  ...
  // 0..1, and every uScale is POSITIVE. Texture space is the panel seen
  // from OUTSIDE, left-to-right, which is the same direction pattern u
  // already runs in — checked per panel against the model, not assumed:
  // front corr(u,X)=+0.99 from +Z, back corr(u,X)=-0.99 from -Z, and each
  // sleeve corr(u,Z)=-/+0.77 from -/+X. All four agree.
  //
  // An earlier version had these NEGATED. That was compensating for a bug
  // in tools/preview_render.py, which built its camera basis with the
  // wrong handedness and so mirrored every render used to verify the
  // convention. Babylon is left-handed: screen-right is cross(forward,up),
  // which puts world +X on the viewer's RIGHT. If the print ever looks
  ...
  const PANELS = {
    ...
    // Names follow the WEARER. Sleeves_Node_7 is world x<0, which renders
    // on the viewer's LEFT, so it is the wearer's RIGHT sleeve.
    sleeve_r: {
      node: "Sleeves_Node_7",
